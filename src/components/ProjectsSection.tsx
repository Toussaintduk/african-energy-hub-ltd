...
const ProjectsSection = () => (
  <section id="projects" className="py-24 bg-energy-dark">
    <div className="container">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
        {stats.map(s => (
          <div key={s.label} className="bg-primary-foreground/5 backdrop-blur rounded-xl p-6 border border-primary/20 text-center">
          ...
          {milestones.map((m, i) => (
            <div key={m.year} className={`relative flex items-start mb-10 last:mb-0 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
              <div className={`hidden md:block w-1/2 ${i % 2 === 0 ? "pr-10 text-right" : "pl-10 text-left"}`}>
                <div className="bg-primary-foreground/5 backdrop-blur rounded-xl p-5 border border-primary/20 inline-block text-left">
                  <h4 className="font-display font-bold text-lg text-primary-foreground">{m.title}</h4>
                  <p className="text-primary-foreground/60 text-sm mt-1">{m.desc}</p>
                </div>
              </div>
              ...
              <div className="ml-14 md:hidden">
                <div className="bg-primary-foreground/5 backdrop-blur rounded-xl p-5 border border-primary/20">
      ...
      <div className="grid md:grid-cols-3 gap-4 max-w-5xl mx-auto">
        {projects.map(p => (
          <div key={p.name} className="bg-primary-foreground/5 backdrop-blur rounded-xl p-5 border border-primary/20 hover:border-secondary/40 transition-all">
