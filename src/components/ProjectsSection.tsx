  ...
  <section id="projects" className="py-24 bg-background">
    <div className="container">
      <div className="text-center mb-16">
        <span className="text-primary font-semibold text-sm uppercase tracking-wider">Our Track Record</span>
        <h2 className="text-3xl md:text-4xl font-display font-bold mt-3">Key Projects & Milestones</h2>
      </div>
      ...
      <div className="max-w-3xl mx-auto mb-16">
        <h3 className="text-2xl font-display font-bold text-center mb-10">Milestones</h3>
          ...
          {milestones.map((m, i) => (
            <div key={m.year} className={`relative flex items-start mb-10 last:mb-0 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
              <div className={`hidden md:block w-1/2 ${i % 2 === 0 ? "pr-10 text-right" : "pl-10 text-left"}`}>
                <div className="bg-card rounded-xl p-5 shadow-card border border-border inline-block text-left">
              ...
              <div className={`hidden md:block w-1/2 ${i % 2 === 0 ? "pl-10" : "pr-10"}`}>
                <span className="text-primary font-display font-bold text-xl">{m.year}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Project List */}
      <h3 className="text-2xl font-display font-bold text-center mb-8">Project Portfolio</h3>
      <div className="grid md:grid-cols-3 gap-4 max-w-5xl mx-auto">
        {projects.map(p => (
          <div key={p.name} className="bg-card rounded-xl p-5 shadow-card border border-border hover:shadow-elevated transition-shadow">
            <div className="flex items-center justify-between mb-2">
              <span className="flex items-center gap-1.5 text-muted-foreground text-xs">
                <MapPin className="h-3 w-3" /> {p.location}
              </span>
              <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${p.status === "completed" ? "bg-accent/20 text-accent" : "bg-secondary/30 text-secondary-foreground"}`}>
